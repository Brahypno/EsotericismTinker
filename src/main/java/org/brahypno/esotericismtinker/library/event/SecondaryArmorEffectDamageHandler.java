        ...
        if (entity instanceof Player player){
            for (ItemStack stack : player.getInventory().items) {
                ...
                if (stack.getItem() instanceof IModifiable){
                    ...
                    amount = ETModifierCheck.modifyDamageTakenInventory(ModifierHooks.MODIFY_DAMAGE, context, source, amount,
                                                                        OnAttackedModifierHook.isDirectDamage(source), ETModifierCheck.toSlot(stack),
                                                                        toolStackView);
                    if (amount <= 0)
                        break;
                }
            ...
            for (ItemStack stack : player.getInventory().items) {
                ...
                if (stack.getItem() instanceof IModifiable){
                    ...
                    if (!toolStack.isBroken()){
                        EquipmentSlot slotType = ETModifierCheck.toSlot(stack);
                        for (ModifierEntry entry : toolStack.getModifierList()) {
                            if (entry.getModifier().is(EsotericismTinkerTagKeys.Modifiers.ArmorWorkingWhenUnequipped) &&
    ...
    static void SecondaryNoneEquipmentHurtHandler(LivingHurtEvent event) {
        ...
        if (entity instanceof Player player){
            for (ItemStack stack : player.getInventory().items) {
                ...
                if (stack.getItem() instanceof IModifiable){
                    IToolStackView toolStackView = ToolStack.from(stack);
                    originalDamage = ETModifierCheck.modifyDamageTakenInventory(ModifierHooks.MODIFY_HURT, context, source, originalDamage,
                                                                                OnAttackedModifierHook.isDirectDamage(source), ETModifierCheck.toSlot(stack),
                                                                                toolStackView);
                    if (originalDamage <= 0)
                        break;
