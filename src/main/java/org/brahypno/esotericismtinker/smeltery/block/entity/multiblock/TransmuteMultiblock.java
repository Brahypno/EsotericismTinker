package org.brahypno.esotericismtinker.smeltery.block.entity.multiblock;
...
public class TransmuteMultiblock extends HeatingStructureMultiblock<TransmuteBlockEntity> {
    ...
    protected boolean isValidBlock(Block block) {
        return block.builtInRegistryHolder().is(EsotericismTinkerTagKeys.Blocks.TRANSMUTE);
    }

    @SuppressWarnings("deprecation")
    @Override
    protected boolean isValidFloor(Block block) {
        return block.builtInRegistryHolder().is(EsotericismTinkerTagKeys.Blocks.TRANSMUTE_FLOOR);
    }

    @SuppressWarnings("deprecation")
    @Override
    protected boolean isValidTank(Block block) {
        return block.builtInRegistryHolder().is(EsotericismTinkerTagKeys.Blocks.TRANSMUTE_TANKS);
    }

    @SuppressWarnings("deprecation")
    @Override
    protected boolean isValidWall(Block block) {
        return block.builtInRegistryHolder().is(EsotericismTinkerTagKeys.Blocks.TRANSMUTE_WALL);
    }
    ...
    protected boolean isValidMeltingSwitch(BlockState state) {
        return state.hasProperty(AshenButtonBlock.Function_Set) &&
               state.getBlock().builtInRegistryHolder().is(EsotericismTinkerTagKeys.Blocks.TRANSMUTE_MELTING_SWITCH);
    }
