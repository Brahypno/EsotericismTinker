...
public final class TranscendenceAnvilScreen
    ...
    public TranscendenceAnvilScreen(TranscendenceAnvilMenu menu, Inventory inventory, Component title) {
        ...
        receptionTypes.addAll(SlotType.getAllSlotTypes().stream().filter(type -> NoumenonDatabase.getReceptionCost(type.getName()) > 0).sorted(Comparator.comparing(type -> type.getDisplayName().getString())).toList());
    }
    ...
    protected void init() {
        ...
        leftLayout.update(leftArea.getX(), leftArea.getY(), leftArea.getWidth(), leftArea.getHeight());
    ...
    private void rebuildLeftWidgets() {
        ...
        if (leftPage == TranscendenceLeftPage.ROOT){
            ...
            for (int index = 0; index < labels.length; index++) {
                ...
                addLeftButton(labels[index], bounds.x(), bounds.y(), bounds.width(), bounds.height(), button -> openLeftPage(target));
            }
            return;
        }

        var backBounds = leftLayout.backButton();
        addLeftButton(Component.literal("<"), backBounds.x(), backBounds.y(), backBounds.width(), backBounds.height(), button -> openLeftPage(TranscendenceLeftPage.ROOT));
    ...
    private void changeReception(SlotType slotType, int delta) {
        ...
        EsotericismTinkerNetwork.CHANNEL.sendToServer(new TranscendenceReceptionPacket(menu.containerId, slotType.getName(), delta));
    }

    private Component validateReceptionChange(String slotType, int delta) {
        LazyToolStack input = menu.getInputTool();
        if (input == null || input.getStack().isEmpty()){
            return Component.translatable("gui.esotericism_tinker.transcendence_anvil.reception.no_preview");
        }
    ...
    private void refreshLeftPanel() {
        ...
        if (shown == null || shown.getStack().isEmpty()){
            leftInfo.setCaption(Component.translatable("gui.esotericism_tinker.transcendence_anvil.positive_aspect"));
    ...
    private static void renderIcon(GuiGraphics graphics, LayoutIcon icon, int x, int y) {
    ...
    protected void renderBg(GuiGraphics graphics, float partialTick, int mouseX, int mouseY) {
        ...
        renderIcon(graphics, stationLayout.getIcon(), (int) (cornerX / scale), (int) (cornerY / scale));
        pose.popPose();
        ...
        PANEL_SPACE_LEFT.draw(graphics, tinkerArea.getX() + 5, tinkerArea.getY() - PANEL_SPACE_LEFT.h);
        PANEL_SPACE_RIGHT.draw(graphics, tinkerArea.getX() + tinkerArea.getWidth() - 5 - PANEL_SPACE_RIGHT.w, tinkerArea.getY() - PANEL_SPACE_RIGHT.h);

        var modifierArea = modifierInfo.getArea();
        PANEL_SPACE_LEFT.draw(graphics, modifierArea.getX() + 5, modifierArea.getY() - PANEL_SPACE_LEFT.h);
        PANEL_SPACE_RIGHT.draw(graphics, modifierArea.getX() + modifierArea.getWidth() - 5 - PANEL_SPACE_RIGHT.w, modifierArea.getY() - PANEL_SPACE_RIGHT.h);
        ...
        for (int index = TranscendenceAnvilMenu.TOOL_SLOT;
            ...
            if (!slot.hasItem()){
                ...
                if (icon != null){
                    GuiUtil.renderPattern(graphics, icon, cornerX + slot.x, cornerY + slot.y);
                }
            }
        }
    ...
    private void renderReceptionContents(GuiGraphics graphics) {
        ...
        Component substrate = data == null
                              ? Component.translatable("gui.esotericism_tinker.transcendence_anvil.substrate_unavailable", pointName)
        ...
        for (int index = receptionScroll; index < end; index++) {
            ...
            Component line = Component.translatable(
                    ...
                    pending,
                    Component.translatable("gui.esotericism_tinker.transcendence_anvil.reception_cost", cost).withStyle(ChatFormatting.DARK_GRAY)
            );
    ...
    private void renderGuiEditMask(GuiGraphics graphics) {
        ...
        for (int index = TranscendenceAnvilMenu.FIRST_MATERIAL_SLOT;
             ...
             index++) {
            Slot slot = menu.getSlot(index);
            graphics.fill(cornerX + slot.x, cornerY + slot.y, cornerX + slot.x + 16, cornerY + slot.y + 16, 0x7F606060);
        }
    }
    ...
    private void rebuildInvestitureWidgets() {
        ...
        try {
            investitureTraits.addAll(NoumenonInvestitureLogic.listDefinitionTraits(source.getTool()));
        }
    ...
    private void selectInvestiture(int traitIndex) {
        if (canSelectInvestiture()){
            EsotericismTinkerNetwork.CHANNEL.sendToServer(new TranscendenceInvestiturePacket(menu.containerId, traitIndex));
        }
    }
    ...
    void renderReceptionScrollbar(GuiGraphics graphics) {
        ...
        graphics.fill(trackX, trackTop, trackX + scrollbar.width(), trackTop + trackHeight, 0xFF303030);
