...
final class TranscendenceLeftStationLayout {
    ...
    Region rootEntry(int index) {
        ...
        return new Region(content.x() + 1, firstY + index * (ROOT_BUTTON_HEIGHT + ROOT_BUTTON_GAP), Math.max(0, content.width() - 2), ROOT_BUTTON_HEIGHT);
    }
    ...
    ReceptionRow receptionRow(int visibleIndex) {
        ...
        return new ReceptionRow(new Region(content.x(), rowTop, content.width(), 12), new Region(content.x() + 1, rowTop, Math.max(0, labelRight - content.x() - 1), 12), new Region(minusX, rowTop, buttonWidth, 12), new Region(plusX, rowTop, buttonWidth, 12));
    }
