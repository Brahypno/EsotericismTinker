...
public final class TranscendenceAnvilMenu extends TabbedContainerMenu<TranscendenceAnvilBlockEntity> {
    ...
    public TranscendenceAnvilMenu(int id, Inventory inventory, @Nullable TranscendenceAnvilBlockEntity station) {
        ...
        addSlot(new Slot(inventory, 40, 132, 74)
                .setBackground(InventoryMenu.BLOCK_ATLAS, InventoryMenu.EMPTY_ARMOR_SLOT_SHIELD)); addInventorySlots(); List<String> slotTypes = SlotType.getAllSlotTypes().stream().map(SlotType::getName).sorted().toList(); for (String slotType : slotTypes) { addDataSlot(new DataSlot() {
                @Override
                public int get() {
                    return station == null
    ...
    public int getInitialReception(String slotType) {
        return syncedInitialReception.getOrDefault(slotType, station == null ? 0 : station.getInitialReception(slotType));
    }
